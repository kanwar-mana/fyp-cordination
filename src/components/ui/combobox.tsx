...
type ComboboxInputSize = "sm" | "default" | "lg";

const comboboxInputGroupSizeVariants: Record<ComboboxInputSize, string> = {
  sm: "h-8",
  default: "h-9",
  lg: "h-12",
};

const comboboxTriggerButtonSizeVariants: Record<
  ComboboxInputSize,
  "icon-xs" | "icon-sm"
> = {
  sm: "icon-xs",
...
function ComboboxInput({
  ...
  return (
    <InputGroup
      className={cn("w-auto", comboboxInputGroupSizeVariants[size], className)}
      data-disabled={disabled ? "true" : undefined}
    >
      {icon ? (
        <InputGroupAddon align="inline-start">
          <span className="text-muted-foreground pointer-events-none [&_svg:not([class*='size-'])]:size-4">
            {icon}
          </span>
        </InputGroupAddon>
      ) : null}
      ...
      <InputGroupAddon align="inline-end">
        {showTrigger && (
          <InputGroupButton
            size={comboboxTriggerButtonSizeVariants[size]}
            variant="ghost"
            asChild
...
function ComboboxItem({
  ...
  return (
    <ComboboxPrimitive.Item
      ...
      className={cn(
        "data-highlighted:bg-accent data-highlighted:text-accent-foreground not-data-[variant=destructive]:data-highlighted:**:text-accent-foreground gap-2 rounded-md py-1 pr-8 pl-1.5 text-sm [&_svg:not([class*='size-'])]:size-4 relative flex w-full cursor-default items-center outline-hidden select-none data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 group-data-[size=sm]/combobox-content:min-h-8 group-data-[size=default]/combobox-content:min-h-9 group-data-[size=lg]/combobox-content:min-h-11",
        size === "sm" && "min-h-8",
